...
@Injectable()
export class FindCompanyInDbService {
  private readonly schema = z.object({
    query: z
      ...
      .max(200)
      .describe(
        'Search term for company name or domain (e.g., "Acme", "acme.com", "software")',
      ),
  });
  ...
  async execute(input: any, state: any): Promise<string> {
    const { query } = input;
    console.log(
      `🚀 [TOOL STARTED] find_company_in_db - Params: ${JSON.stringify({ query })} - User: ${state.user_id}`,
    );
    ...
    try {
      ...
      // 3. Search Companies (fuzzy match on name or domain)
      ...
      if (companies.length === 0) {
        // Try to suggest similar companies
        const similarCompanies = await prisma.company.findMany({
          where: {
            ...
            OR: [
              {
                name: {
                  contains: searchTerm.substring(
                    0,
                    Math.max(3, Math.floor(searchTerm.length / 2)),
                  ),
                  mode: 'insensitive',
                },
              },
...
- Use \`get_companies_list\` to browse all companies`;
      ...
      const companiesInCampaigns = companies.filter(
      ...
      const results = companies
        .map((c, index) => {
          ...
          const leadInfo =
            c._count.leads > 0 ? ` | ${c._count.leads} leads` : ' | No leads';
          const campaignInfo =
