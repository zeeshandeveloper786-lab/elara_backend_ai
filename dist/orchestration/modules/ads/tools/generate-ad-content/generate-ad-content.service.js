...
exports.GenerateAdContentService = void 0;
const common_1 = require("@nestjs/common");
const openai_1 = require("@langchain/openai");
const zod_1 = require("zod");
const messages_1 = require("@langchain/core/messages");
const prisma_client_1 = require("../../../../../prisma-client");
let GenerateAdContentService = class GenerateAdContentService {
    chatModel;
    generateAdContentSchema = zod_1.z.object({
        platform: zod_1.z
            .enum(['Facebook', 'Instagram', 'LinkedIn', 'Google Ads', 'Twitter'])
            .describe('The advertising platform to optimize the ad for'),
        core_message: zod_1.z
            .string()
        ...
        target_audience: zod_1.z
            .string()
            .optional()
            .describe('Target audience description (e.g., "B2B SaaS founders", "E-commerce store owners")'),
        ad_title: zod_1.z
            .string()
            .optional()
    ...
    async execute({ platform, core_message, target_audience, ad_title }, state) {
        console.log(`🚀 [TOOL STARTED] generate_ad_content - Params: ${JSON.stringify({ platform, core_message, target_audience, ad_title })} - User: ${state.user_id}`);
        const { user_id, company_id, campaign_id } = state;
        console.log(`🚀 [TOOL STARTED] generate_ad_content - Platform: ${platform} - User: ${user_id}`);
        console.log(`📣 [generate_ad_content] Starting ad generation for platform: ${platform}`);
...
**Important:** Stay within character limits and make every word count!`;
            console.log(`🤖 [generate_ad_content] Invoking AI model for ${platform} ad generation`);
            const aiResponse = await this.chatModel.invoke([
                new messages_1.SystemMessage(copywriterPrompt),
            ]);
            ...
            const hasStructure = content.includes('Headline:') &&
                content.includes('Body:') &&
                content.includes('CTA:');
            if (!hasStructure) {
                console.warn('⚠️ [generate_ad_content] AI response missing expected structure');
            }
            console.log(`💾 [generate_ad_content] Saving ad to database`);
            const ad = await prisma_client_1.prisma.adContent.create({
                data: {
                    title: ad_title || `${platform} Ad - ${new Date().toLocaleDateString()}`,
                    body: content,
                    platform,
            ...
            console.log(`✅ [generate_ad_content] Ad created successfully with ID: ${ad.id}`);
...
exports.GenerateAdContentService = GenerateAdContentService;
exports.GenerateAdContentService = GenerateAdContentService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [openai_1.ChatOpenAI])
], GenerateAdContentService);
//# sourceMappingURL=generate-ad-content.service.js.map